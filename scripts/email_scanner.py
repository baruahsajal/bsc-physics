        ...
        for e_id in email_ids:
            ...
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    ...
                    # Example Regex logic (Adjust based on your actual bank/UPI email formats)
                    # Looks for: "Received ₹500 from Pranku Kurmi"
                    amount_match = re.search(r'₹(\d+)', payload)
                    name_match = re.search(r'from\s+([A-Za-z\s]+)', payload)
                    ...
                    if amount_match and name_match:
                        ...
                        ledger.append({
                            "id": "SYS-AUTO", # You can map this to actual IDs if known
                            "name": name,
                            "amount": amount,
